...
# def subsample(data, sample_p):
#     '''
#     sampling for local training
#     '''
...
#     sample_split_idx = int(np.floor(sample_p * n))
#     sample_idx = idx[:sample_split_idx]

#     subsampler = SubsetRandomSampler(sample_idx)
    
#     subsample_loader = DataLoader(data, shuffle=False,
#                                   batch_size=args.batch_size,
#                                   sampler=subsampler)
                                
#     return subsample_loader, sample_idx
...
#         subsample.append(data[idx])         
#     return subsample, sample_idx




def subsample(args, dataset_dict):
    '''
    sampling for local training
    '''
    n = len(dataset_dict['id'])
    idx = list(range(n))
    np.random.shuffle(idx)

    sample_split_idx = int(np.floor(args.sample_p * n))
    sample_idx = idx[:sample_split_idx]

    # subsample_id = [dataset_dict['id'][i] for i in sample_idx]
    
    sample_dict = dict()
    for key in dataset_dict.keys():
        sample_dict[key] = [dataset_dict[key][i] for i in sample_idx]
